...
public class ContextCapabilities {
    ...
    // Additional caps queried by Minecraft snooper/log
    ...
    public ContextCapabilities() {
        try {
            ...
            for (Field target : ContextCapabilities.class.getFields()) {
                if (target.getType() != boolean.class) {
                    continue;
                }

                try {
                    Field source = GL_CAPABILITY_FIELDS.get(target.getName());
                    if (source != null) {
                        target.setBoolean(this, source.getBoolean(caps));
                    } else {
                        target.setBoolean(this, false);
                    }
    ...
    private static Map<String, Field> buildGLCapabilityFieldMap() {
