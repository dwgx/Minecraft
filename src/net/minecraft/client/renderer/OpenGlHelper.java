...
public class OpenGlHelper
{
    ...
    public static void initializeTextures()
    {
        ...
        String s = String.valueOf(GL11.glGetString(GL11.GL_VENDOR)).toLowerCase();
    ...
    {
        ByteBuffer bytebuffer = string.duplicate();
        byte[] abyte = new byte[bytebuffer.remaining()];
        bytebuffer.get(abyte);
        String s = new String(abyte, StandardCharsets.UTF_8);

        if (arbShaders)
        {
            ARBShaderObjects.glShaderSourceARB(shaderIn, s);
        }
        else
        {
            GL20.glShaderSource(shaderIn, s);
        }
    }
